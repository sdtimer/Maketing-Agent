...
import { guardShell } from '@/utils/shellAccess'
  ...
  if (getAccessToken()) {
    if (to.path === '/login') {
      ...
      if (!userStore.getIsSetUser) {
        ...
        const jump = guardShell(nextData.path || to.path, userStore.getUser.username, userStore.getRoles)
        next(jump ? { path: jump, replace: true } : nextData)
      } else {
        const jump = guardShell(to.path, userStore.getUser.username, userStore.getRoles)
        if (jump) next({ path: jump, replace: true })
        else next()
      }
    }
  } else {
