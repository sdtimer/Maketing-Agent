...
/** 顶级目录按 path 选壳。产品菜单仍走 system_menu，不另写一套路由表。 */
const rootLayout = (path?: string) => {
  if (path === '/tenant') return TenantLayout
  if (path === '/admin' || path === '/system') return AdminLayout
  return Layout
}
  ...
  for (const route of routes) {
    ...
    if (!route.children && Number(route.parentId) === 0 && route.component) {
      ...
      if (route.children?.length) {
        ...
        data.component = Number(route.parentId) === 0 ? rootLayout(route.path) : getParentLayout()
