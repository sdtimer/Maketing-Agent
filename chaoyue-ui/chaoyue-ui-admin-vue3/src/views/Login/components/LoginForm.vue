<template>
  ...
  >
    <el-row class="mx-[-10px]">
      ...
      <el-col :span="24" class="px-10px">
        <p class="m-0 text-12px leading-5 text-[var(--el-text-color-secondary)]">
          演示账号密码均为 admin123。<br />
          租户「芋道源码」：admin / platform<br />
          租户「超悦内部」：oper / reviewer<br />
          租户「演示租户B」：tenantb
        </p>
      </el-col>
      <Verify
...
const handleLogin = async (params: any) => {
  ...
  try {
    ...
    if (!redirect.value || redirect.value === '/') {
      const u = loginData.loginForm.username
      redirect.value = u === 'platform' || u === 'admin' ? '/admin/home' : '/tenant/home'
    }
    // 判断是否为SSO登录
