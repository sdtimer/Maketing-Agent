...
CREATE TABLE IF NOT EXISTS `marketing_creation_task` (`id` bigint NOT NULL AUTO_INCREMENT,`title` varchar(256) NOT NULL,`status` varchar(32) NOT NULL,`market` varchar(32) NOT NULL,`fact_snapshot` mediumtext NOT NULL,`tenant_id` bigint NOT NULL,`creator` varchar(64) DEFAULT '',`create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,`updater` varchar(64) DEFAULT '',`update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,`deleted` bit(1) NOT NULL DEFAULT b'0',PRIMARY KEY (`id`),KEY `idx_tenant_status` (`tenant_id`,`status`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='智造营销-推广创作任务';
