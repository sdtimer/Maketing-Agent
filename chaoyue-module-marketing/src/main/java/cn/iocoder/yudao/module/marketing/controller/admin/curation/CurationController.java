...
public class CurationController {
    ...
    public CommonResult<Boolean> publish(@PathVariable Long id) { return changeStatus(id, "published"); }
    @PostMapping("/batch/{id}/offline") @PreAuthorize("@ss.hasPermission('marketing:platform:write')")
    public CommonResult<Boolean> offline(@PathVariable Long id) { return changeStatus(id, "offline"); }
    private CommonResult<Boolean> changeStatus(Long id, String status) { CurationBatchDO batch=batchMapper.selectById(id); if(batch==null) throw exception(MARKETING_INGESTION_NOT_FOUND); batch.setStatus(status); if("published".equals(status)) batch.setPublishedAt(LocalDateTime.now()); batchMapper.updateById(batch); return success(true); }
}
